#!/usr/bin/env python3
"""Kobeissi Letter Daily Briefings podcast feed.

Scans ~/workspace/kobeissi-voice-note/runs/ for new briefing MP3s, uploads
them to catbox.moe, appends them as episodes to feed.xml, and pushes to the
podcast-feeds GitHub repo (public URL:
https://moretoexplore1234.github.io/podcast-feeds/kobeissi-briefings/feed.xml).
...
FEED_FILE = "feed.xml"
SITE_PATH = "kobeissi-briefings"
GUID_PREFIX = "kobeissi"
ET = ZoneInfo("America/New_York")
SLOT_TIME = {"am": (7, 0), "pm": (16, 30)}        # approximate run time, ET
SLOT_LABEL = {"am": "Morning briefing", "pm": "Afternoon briefing"}
# ---------------------------------------------
...
def ensure_artwork(state):
    ...
    if not os.path.exists(ART_PATH):
        ...
        return None
    url = catbox_upload(ART_PATH, "kobeissi-briefings-artwork.jpg")
    state["artwork"] = url
    save_state(state)
...
def add_episode(state, key, mp3, date_s, slot, addendum):
    ...
    url = catbox_upload(mp3, f"kobeissi-{date_s}-{slot}.mp3")
...
def build_feed(state):
    ...
    parts = ['<?xml version="1.0" encoding="UTF-8"?>',
             ...
             "<link>https://x.com/KobeissiLetter</link>",
             ...
             (f"<image><url>{esc(art)}</url><title>{esc(SHOW_TITLE)}</title>"
              f"<link>https://x.com/KobeissiLetter</link></image>") if art else ""]
    for e in items:
        parts.append("<item>")
...
def git_push():
    ...
    subprocess.run(["git", "commit", "-m",
                    f"Kobeissi briefings: feed update ({n} changed)"],
